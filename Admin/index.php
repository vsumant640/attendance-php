...
include '../Includes/session.php';
    ...
    $topStudentsQuery = "SELECT
      ...
      COALESCE(ROUND((SUM(CASE WHEN a.status = '1' THEN 1 ELSE 0 END) / NULLIF(COUNT(a.Id), 0)) * 100, 2), 0) AS attendanceRate
    FROM tblstudents s
    ...
    GROUP BY s.Id, s.firstName, s.lastName, s.otherName, s.admissionNumber, c.className, ca.classArmName
    ORDER BY attendanceRate DESC, totalCount DESC, s.firstName ASC
    LIMIT 10";

    $topStudentsRs = $conn->query($topStudentsQuery);
            ...
            <div class="col-lg-12">
              <div class="card">
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                  <h6 class="m-0 font-weight-bold text-primary">Top Students For Prize (By Attendance)</h6>
                  <small class="text-muted">Student Name + Admission ID included</small>
                </div>
                <div class="table-responsive p-3">
                  <table class="table align-items-center table-flush table-hover">
                    ...
                    <tbody>
                      <?php
                      if ($topStudentsRs && $topStudentsRs->num_rows > 0) {
                        $rank = 1;
                        while ($student = $topStudentsRs->fetch_assoc()) {
                          ...
                          echo "<tr>
                                  ...
                                  <td>".htmlspecialchars($student['className'].' - '.$student['classArmName'])."</td>
                                  <td>".(int)$student['presentCount']."</td>
                                  <td>".(int)$student['totalCount']."</td>
                                  <td><span class='badge badge-success'>".(float)$student['attendanceRate']."%</span></td>
                                </tr>";
                          $rank++;
                        }
                      } else {
                        echo "<tr><td colspan='7' class='text-center text-muted'>No student attendance data found yet.</td></tr>";
                      }
                      ?>
                    </tbody>
                  </table>
